  ...
  if (is_type == "true") {
    ...
    if (n_components == 0) {
      ...
      if (has_capability&(has_accuracy|has_measurement_range|has_precision|has_resolution|has_sensitivity|has_frequency)) {
        sensor_capability <- paste0("<http://rdfdata.get-it.it/sensors/systemsType/", uuid, "#SensorCapability>")
        if (has_accuracy) {
      ...
      for (i in 1:length(components_files)) {
        ...
        comp_id <- stringr::str_replace(as.character(
          ...
          "^.+/", "")
        RDF_comp_uri <- paste0("http://rdfdata.get-it.it/sensors/componentsType/", comp_id) #TODO add model name and manufacturer name (e.g. http://rdfdata.get-it.it/sensors/componentsType/Vantage/Davis_Instruments/uuid)
          ...
          comp_sensor_capability_02 <- c(
            ...
            gsub("([,&\\s])\\1+", "\\1",
                 paste0(
                   "ssn-system:hasSystemProperty ",
                   paste(
                     comp_accuracy_uri, comp_measurement_range_uri,
      ...
      components <- paste0(
        ...
        paste(
          "<",
          paste0(
            "http://rdfdata.get-it.it/sensors/componentsType/", #TODO add model name and manufacturer name (e.g. http://rdfdata.get-it.it/sensors/componentsType/Vantage/Davis_Instruments/uuid)
            sub('.*\\/', '', components_uuid)
          ),
          ">",
          sep = "",
    ...
    ttl_files <- list.files(path = dir_name, pattern = "\\.ttl$")
    for (j in 1:length(ttl_files)) {
      new_sensorType_qr <- httr2::request("http://fuseki1.get-it.it/systemsType") |>
        ...
        httr2::req_body_file(
          path = paste0(dir_name, "/", ttl_files[j]),
          type = "text/turtle"
        ) |>
