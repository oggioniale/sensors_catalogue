...
sensors_catalogue <- function(excel_path = NULL) {
  ...
  man_exist <- check_man_exist(
    manufacturer_name = excel_file$data[[1]]$manufacturer[1]
  )
  if (length(man_exist) == 1) {
    ...
    stop("\n----\nManufacturer is not registered in the triplestore.
...
Or the use of rdf_ttl_manufacturer() function for bash creation of multiple record in triple store.\n----\n")
  } else if (length(man_exist) > 0) {
    stop("\n----\nIn the triplestore there are ", length(man_exist), " manufacturers. ",
"Please change the name in the excel selecting among one of this: ", paste(man_exist, collapse = ", "), ".\n----\n")
  }
}
