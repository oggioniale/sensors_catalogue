...
sensorML_compTypeXML <- function(sensorList, uuidsList) {
  ...
  for (z in 1:nrow(components_info)) {
    components_info <- components_info[z,]
    ...
    # gml:description
    if (!is.na(components_info$description)) {
      xml2::xml_add_child(c, "gml:description", components_info$description)
    }
    ...
    # gml:name
    if (!is.na(components_info$name)) {
      xml2::xml_add_child(c, "gml:name", components_info$name)
    }
    
    # sml:keywords
    if (!is.na(components_info$keywords)) {
      c1 <- xml2::xml_add_child(c, "sml:keywords") %>%
        xml2::xml_add_child(., "sml:KeywordList")
      list_keywords <- stringr::str_split(components_info$keywords, pattern = ",")
    ...
    if (!is.na(components_info$short_name)) {
      xml2::xml_add_child(d, "sml:identifier") %>%
        ...
        xml2::xml_add_sibling(., "sml:value", components_info$short_name)
    }
    
    # sml:classification
    if (!is.na(components_info$sensor_type)) {
  ...
  WHERE {
    SERVICE <http://vocab.nerc.ac.uk/sparql/sparql> {
      ...
      xml2::xml_add_child(e, "sml:classifier") %>%
        ...
        xml2::xml_add_child(., "sml:label", components_info$sensor_type) %>%
  ...
  WHERE {
    ...
    if (!is.na(components_info$accuracy)) {
      uom_nerc_query <- paste0(
        uom_query_I_part,
        components_info$accuracy_uom,
        uom_query_II_part
      )
      ...
      if (!is.na(as.numeric(components_info$accuracy))) {
        xml2::xml_add_child(g, "sml:capability", "name" = "Accuracy") %>%
          ...
          xml2::xml_add_child(., "swe:uom", "code" = components_info$accuracy_uom, "xlink:href" = accuracy_uom_def) %>%
          xml2::xml_add_sibling(., "swe:value", components_info$accuracy)
      } else {
        if (grepl("\\s+", components_info$accuracy)) {
          xml2::xml_add_child(g, "sml:capability", "name" = "Accuracy") %>%
          ...
          xml2::xml_add_child(g, "sml:capability", "name" = "Accuracy") %>%
            ...
            xml2::xml_add_child(., "swe:value", components_info$accuracy)
        }
      }
    }
    # sml:capabilities - MeasurementRange
    if (!is.na(components_info$measurement_range)) {
      uom_nerc_query <- paste0(
        uom_query_I_part,
        components_info$accuracy_uom,
        uom_query_II_part
      ...
      xml2::xml_add_child(g, "sml:capability", "name" = "MeasurementRange") %>%
        ...
        xml2::xml_add_child(., "swe:uom", "code" = components_info$measurement_range_uom, "xlink:href" = measurement_range_uom_def) %>%
        xml2::xml_add_sibling(., "swe:value", components_info$measurement_range)
    }
    # sml:capabilities - OperatingDepth
    if (!is.na(components_info$operating_depth)) {
      xml2::xml_add_child(g, "sml:capability", "name" = "OperatingDepth") %>%
        ...
        xml2::xml_add_sibling(., "swe:value", components_info$operating_depth)
    }
    # sml:capabilities - Precision
    if (!is.na(components_info$precision)) {
      uom_nerc_query <- paste0(
        uom_query_I_part,
        components_info$accuracy_uom,
        uom_query_II_part
      ...
      xml2::xml_add_child(g, "sml:capability", "name" = "Precision") %>%
        ...
        xml2::xml_add_child(., "swe:uom", "code" = components_info$precision_uom, "xlink:href" = precision_uom_def) %>%
        xml2::xml_add_sibling(., "swe:value", components_info$precision)
    }
    # sml:capabilities - Resolution
    if (!is.na(components_info$resolution)) {
      uom_nerc_query <- paste0(
        uom_query_I_part,
        components_info$accuracy_uom,
        uom_query_II_part
      ...
      if (!is.na(as.numeric(components_info$resolution))) {
        xml2::xml_add_child(g, "sml:capability", "name" = "Resolution") %>%
          ...
          xml2::xml_add_child(., "swe:uom", "code" = components_info$resolution_uom, "xlink:href" = resolution_uom_def) %>%
          xml2::xml_add_sibling(., "swe:value", components_info$resolution)
      } else {
        if (grepl("\\s+", components_info$resolution)) {
          xml2::xml_add_child(g, "sml:capability", "name" = "Resolution") %>%
    ...
    if (!is.na(components_info$sensitivity)) {
      uom_nerc_query <- paste0(
        uom_query_I_part,
        components_info$accuracy_uom,
        uom_query_II_part
      ...
      xml2::xml_add_child(g, "sml:capability", "name" = "Sensitivity") %>%
        ...
        xml2::xml_add_child(., "swe:uom", "code" = components_info$sensitivity_uom, "xlink:href" = sensitivity_uom_def) %>%
        xml2::xml_add_sibling(., "swe:value", components_info$sensitivity)
    }
    # sml:capabilities - MinimumReportingFrequency
    if (!is.na(components_info$minimum_reporting_frequency)) {
      xml2::xml_add_child(g, "sml:capability", "name" = "MinimumReportingFrequency") %>%
        ...
        xml2::xml_add_sibling(., "swe:value", components_info$minimum_reporting_frequency)
    }
    # sml:capabilities - Mobile
    if (!is.na(components_info$mobile)) {
      xml2::xml_add_child(g, "sml:capability", "name" = "Mobile") %>%
        xml2::xml_add_child(., "swe:Boolean") %>%
        xml2::xml_add_child(., "swe:value", tolower(components_info$mobile))
    }
    # sml:capabilities - Insitu
    if (!is.na(components_info$insitu)) {
      xml2::xml_add_child(g, "sml:capability", "name" = "Insitu") %>%
        xml2::xml_add_child(., "swe:Boolean") %>%
        xml2::xml_add_child(., "swe:value", tolower(components_info$insitu))
    }
    ...
    LIMIT 1")
      ...
      xml2::xml_add_child(h, "sml:contact", "xlink:title" = sensorList$manufacturer[1], "xlink:href" = manufacturer_rdf) %>%
        xml2::xml_add_child(., "gmd:CI_ResponsibleParty") %>%
        xml2::xml_add_child(., "gmd:role") %>%
        xml2::xml_add_child(., "gmd:CI_RoleCode",
                            "codeList" = "http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_RoleCode/",
                            "codeListValue" = "originator", sensorList$manufacturer[1]
        )
    }
  ...
  #   if (!is.na(components_info$observed_property)) {
  ...
  #     }
  #     FILTER( REGEX( STR(?l), '",
  #     components_info$observed_property,
  #     "', 'i'))
  #      }
  #     }
  #     ORDER BY ASC(?l)
